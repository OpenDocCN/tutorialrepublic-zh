 ...
 ***注意:**您也可以通过将 CSS [`border-spacing`](../css-reference/css-border-spacing-property.php) 属性的值设置为 0 来删除表格单元格边框之间的空格。然而，它只是移除了空间，而没有像你设置`border-collapse`为`collapse`时那样合并边界。*  ** * *
