...
> 原文:[https://www . tutorial Republic . com/PHP-reference/PHP-vprintf-function . PHP](https://www.tutorialrepublic.com/php-reference/php-vprintf-function.php)

题目: [PHP 字符串引用](php-string-functions.php) [上一个](php-vfprintf-function.php) | [下一个](php-vsprintf-function.php)
...
相关功能:[`fprintf()`](php-fprintf-function.php)[`printf()`](php-printf-function.php)[`sprintf()`](php-sprintf-function.php)[`vfprintf()`](php-vfprintf-function.php)[`vsprintf()`](php-vsprintf-function.php)。
...
vprintf(*format*, *argument_array*);

下面的例子展示了`vprintf()`函数的作用。

#### 例子

[Run this code »](../codelab.php?topic=php&file=output-a-formatted-string-using-vprintf "Run this code to view the output")
 ...
 ***注意:***格式*字符串中的每个字符都将被逐字打印，除了%字符和紧随其后的字符。字符%表示一个[转换规范](#conversion-specification)的开始，它指定如何打印跟随*格式*字符串的参数。*  ****提示:**`vprintf()`功能与`printf()`相同，只有一点例外。`vprintf()`函数接受一个参数数组，而`printf()`接受一个参数列表。*  ** * *
...
转换规格以百分号(%)开始。对于在*格式*字符串之后传递给`vprintf()`函数的每个参数，必须包含一个转换规范。
...
这里有更多的例子来展示`vprintf()`函数实际上是如何工作的:

以下示例显示了如何将多个转换规范应用于同一个参数。

#### 例子

[Run this code »](../codelab.php?topic=php&file=apply-multiple-placeholders-to-same-argument-with-vprintf "Run this code to view the output")
...
[Run this code »](../codelab.php?topic=php&file=format-numbers-with-decimal-points-using-vprintf "Run this code to view the output")
...
默认情况下，每个转换规范将按照它们在数组中的排列顺序被格式化的参数替换。但是，您可以使用 *argnum* 交换格式字符串中的参数。

#### 例子

[Run this code »](../codelab.php?topic=php&file=swap-arguments-inside-format-string-with-vprintf "Run this code to view the output")
 ...
 ***提示:**如果格式字符串中占位符或转换规范的顺序与数组中列出的参数顺序不匹配。您可以使用 *argnum* 轻松地指出占位符引用了哪些参数，并保持函数代码不变。*  *下面的例子简单地展示了当使用`printf()`函数对整数值使用不同的格式说明符时可能的结果。
