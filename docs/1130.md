...
Bootstrap 提供了一种简单快捷的方式来创建基本的导航组件，如标签和药丸，非常灵活和优雅。所有 Bootstrap 程序的导航组件——标签和药丸——通过基类`.nav`共享相同的基本标签和样式。
...
## 带有下拉菜单的 Bootstrap 标签和药丸导航

你可以添加下拉菜单到标签页和药丸导航的链接中，只需要一点额外的标签。
